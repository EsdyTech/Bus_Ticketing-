			...
			<div id="panel" class="radius">
				...
				<div id="content" class="clearfix">
					...
					<table cellpadding="1" cellspacing="1" id="resultTable">
						...
						<?php
							include('../db.php');
							$result = mysqli_query($con,"SELECT customer.lname, customer.fname, route.type, route.route, reserve.id, reserve.date, reserve.seat, 
							reserve.transactionnum, reserve.payable, reserve.status from reserve
							 inner join customer on customer.id = reserve.cusId
							 inner join route on route.id = reserve.bus");
							while($row = mysqli_fetch_array($result))
								{
									echo '<td><div align="right">'.$row['lname'].' '.$row['fname'].'</div></td>';
									echo '<td style="border-left: 1px solid #C1DAD7"><div align="right">'.$row['date'].'</div></td>';
									...
									echo '<td><div align="center"><a rel="facebox" href="editstatus.php?id='.$row['id'].'">edit</a> | <a href="#" id="'.$row['id'].'" class="delbutton" title="Click To Delete">delete</a></div></td>';
									echo '</tr>';
								}
							?> 
						</tbody>
